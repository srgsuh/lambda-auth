...
def refresh_token(client, refresh_token: str) -> dict:
    try:
        response = client.initiate_auth(
...
def refresh(event: dict) -> dict:
    token = parse_refresh_request(event).refresh_token
    return refresh_token(CLIENT, token)

def get_route(event: dict) -> tuple[str | None, str | None]:
    context: dict = event.get("requestContext", {})
    http: dict = context.get("http", {})
...
def get_handler(method: str | None, path: str | None) -> Callable[[dict], dict]:
    key = (method, path)
    handler = ROUTES.get(key)
    if not handler:
        raise InternalException(f"Route not implemented: {key}")
...
def lambda_handler(event, context) -> dict:
    try:
        ...
        logger.debug(f"Route: {method} -> {path}")
        handler = get_handler(method, path)
        auth_response = handler(event)
        logger.debug(f'Auth response: {auth_response}')
        reply = auth_response.get(AUTH_RESULT, {})
